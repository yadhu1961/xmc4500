...
void sendLine(InBuffer *buffer)
{
    ...
    memcpy(outbuf, buffer->buffer, charsInLine);
    ...
    for(ssize_t c = charsInLine; c > 0 && c < buffer->count; ++c) {
        buffer->buffer[c-charsInLine] = buffer->buffer[c];
    }
    buffer->count -= charsInLine;
    /* send line to host */
    CDC_Device_SendData(&VirtualSerial_CDC_Interface, outbuf, charsInLine);
}
